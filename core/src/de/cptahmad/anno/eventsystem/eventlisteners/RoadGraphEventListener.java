...
import de.cptahmad.anno.eventsystem.events.RoadGraphEvent;
...
public class RoadGraphEventListener extends EventListener
{
    private final Graph<Point2DInteger, DefaultEdge> m_graph;
    private final List<Building> m_buildings;

    public RoadGraphEventListener(Graph<Point2DInteger, DefaultEdge> graph, List<Building> buildings)
    {
        super();
        m_graph = graph;
    ...
    {
        if(event.getClass() != RoadGraphEvent.class) return;

        RoadGraphEvent e = (RoadGraphEvent) event;
